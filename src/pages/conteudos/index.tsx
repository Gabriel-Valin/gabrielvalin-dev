    ...
    return (
      <Container>
        <ContainerSection>
          {postsPublisheds?.map((post: Posts) => (
            <ContainerPost key={post.slug}>
              <Link href={`conteudos/${post.slug}`} passHref>
                <a key={post.slug}>
                  <Image width={720} height={420} quality={100} src={post.cover} alt={post.title} placeholder='blur' blurDataURL='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mN8Ool3EwAGIAI4bTwgvAAAAABJRU5ErkJggg==' />
                </a>
              </Link>
              <ContainerPostBody>
                <TitlePost>
          ...
          <Pagination pageNumber={page} totalPage={totalPages} setPosts={setPostsPublisheds} />
        </ContainerSection>
      </Container>
    );
}
